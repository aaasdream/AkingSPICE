...
exports.GeneralizedAlphaFactory = exports.GeneralizedAlphaIntegrator = void 0;
...
class GeneralizedAlphaIntegrator {
    ...
    get history() {
        ...
        if (this._currentState)
            states.push(this._currentState);
        if (this._previousState)
            states.push(this._previousState);
        return states;
    }
    /**
    ...
    restart(initialState) {
        ...
        const velocity = initialState.derivative || new vector_1.Vector(initialState.solution.size);
        const acceleration = new vector_1.Vector(initialState.solution.size); // 零初始加速度
        this._currentState = {
            ...initialState,
            velocity,
            acceleration,
     ...
     * 初始化首步狀態
     */
    _initializeFirstStep(_system, // 參數保留但未使用
    t0, v0) {
        // 計算初始速度 v₀ = dv/dt|₀  
        // 簡化假設: dv/dt|₀ ≈ 0 (靜態初始條件)
        const initialVelocity = new vector_1.Vector(v0.size);
        // 計算初始加速度 a₀ = d²v/dt²|₀
        // 對於電路: C * dv/dt + G * v = I(t) 
        const initialAcceleration = new vector_1.Vector(v0.size);
        return {
            time: t0,
            solution: v0.clone(),
     ...
     * Newton 修正迭代
     */
    _correctStep(system, t_n1, dt, predicted) {
     ...
     */
    _buildGeneralizedAlphaResidual(system, _t_n1, // 參數保留但未使用  
    _dt, // 參數保留但未使用
    v_n1, vel_n1, _acc_n1 // 參數保留但未使用
    ) {
        const curr = this._currentState;
        // Generalized-α 公式:
        ...
        const residual = new vector_1.Vector(v_n1.size);
     ...
     */
    _buildGeneralizedAlphaJacobian(system, dt) {
     ...
     */
    _solveNewtonStep(jacobian, residual) {
        ...
        try {
            ...
            if (jacobian && typeof jacobian.solve === 'function') {
                const negResidual = new vector_1.Vector(n);
                for (let i = 0; i < n; i++) {
            ...
            const delta = new vector_1.Vector(n);
            for (let i = 0; i < n; i++) {
     ...
     */
    _updateVelocityAcceleration(dt, deltaV, vel_n1, acc_n1) {
        const curr = this._currentState;
        // Newmark-β 更新公式:
     ...
     * 估計局部截斷誤差
     */
    _estimateLocalTruncationError(corrected, predicted) {
        // 使用預測-校正差值估計 LTE
     ...
     * 自適應步長調整 (PI 控制器)
     */
    _adjustTimestep(dt, lte, accepted) {
        if (lte < 1e-15) {
            // 誤差極小，允許較大步長增長
            return dt * 1.5;
        }
...
exports.GeneralizedAlphaIntegrator = GeneralizedAlphaIntegrator;
/**
 ...
 */
class GeneralizedAlphaFactory {
    /**
     * 创建稳定性优先的积分器
     * 适用场景：含开关器件的硬非线性电路
     */
    static createStable(options = {}) {
...
exports.GeneralizedAlphaFactory = GeneralizedAlphaFactory;
