...
exports.BuckConverterAnalyzer = exports.BuckConverterCircuit = exports.runBuckConverterSimulation = void 0;
...
const intelligent_device_factory_1 = require("./src/core/devices/intelligent_device_factory");
const inductor_1 = require("./src/components/passive/inductor");
const capacitor_1 = require("./src/components/passive/capacitor");
...
class BuckConverterCircuit {
    constructor(inputVoltage = 12, // Input voltage (V)
    outputCurrent = 2, // Desired output current (A)
    switchingFrequency = 100e3, // Switching frequency (Hz)
    dutyCycle = 0.417 // PWM duty cycle (for ~5V output)
    ) {
    ...
    buildCircuit() {
        ...
        const inputSource = voltage_source_1.VoltageSourceFactory.createDC('Vin', ['vin', 'gnd'], this.inputVoltage);
        this.components.push(inputSource);
        // 2. PWM gate drive signal
        const gateDriver = voltage_source_1.VoltageSourceFactory.createPulse('Vgate', // Name
        ...
        const mainSwitch = intelligent_device_factory_1.SmartDeviceFactory.createBuckMOSFET('M1', [nodeMap.get('vin'), nodeMap.get('gate'), nodeMap.get('sw')], // [Drain, Gate, Source]
        this.inputVoltage, this.outputCurrent);
        this.components.push(mainSwitch);
        // 4. Freewheel diode (for continuous current path)
        const freewheelDiode = intelligent_device_factory_1.SmartDeviceFactory.createFreewheelDiode('D1', [nodeMap.get('gnd'), nodeMap.get('sw')], // [Anode, Cathode]
        this.inputVoltage, this.outputCurrent);
        this.components.push(freewheelDiode);
        // 5. Filter inductor (47µH - standard for 100kHz switching)
        const filterInductor = new inductor_1.Inductor('L1', ['sw', 'out'], 47e-6); // 47µH
        this.components.push(filterInductor);
        // 6. Output filter capacitor (100µF - low ESR ceramic or electrolytic)
        const outputCapacitor = new capacitor_1.Capacitor('C1', ['out', 'gnd'], 100e-6); // 100µF
        this.components.push(outputCapacitor);
        // 7. Load resistor (2.5Ω for 2A @ 5V)
        const loadResistor = new resistor_1.Resistor('R1', ['out', 'gnd'], 2.5); // 2.5Ω
...
exports.BuckConverterCircuit = BuckConverterCircuit;
/**
...
class BuckConverterAnalyzer {
     ...
     */
    analyzeResults(result, nodeMapping) {
        if (!result.success) {
            console.error("\n❌ Simulation Failed!");
     ...
     */
    analyzeOutputVoltage(result, nodeMapping) {
        ...
        const averageVoltage = steadyStateVoltages.reduce((sum, v) => sum + v, 0) / steadyStateVoltages.length;
        // Calculate ripple
...
exports.BuckConverterAnalyzer = BuckConverterAnalyzer;
/**
...
async function runBuckConverterSimulation() {
    ...
    try {
        ...
        const circuit = new BuckConverterCircuit(12, // 12V input
        2, // 2A output current
        ...
        const engine = new circuit_simulation_engine_1.CircuitSimulationEngine(simConfig);
...
exports.runBuckConverterSimulation = runBuckConverterSimulation;
/**
